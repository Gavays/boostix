...
function App() {
  ...
  const handlePlatformClick = (platformId) => { setSelectedPlatform(platformId); setSelectedService(null); setStep('services') }
  const handleServiceClick = (service) => { setSelectedService(service); setStep('order') }
  const handleBack = () => {
      ...
      <div className="tab-content">
            ...
            {step === 'platforms' && (
              <>
                ...
                <div className="platforms-grid">
                  {PLATFORMS.map(p => (
                    <div key={p.id} className="platform-card" onClick={() => handlePlatformClick(p.id)}>
              ...
              <>
                ...
                <div className="services-scroll">
                  {filteredServices.map((s, i) => (
                    <div key={i} className="service-detail-card" onClick={() => handleServiceClick(s)}>
                      <div className="service-detail-name">{s.name}</div>
                      <div className="service-detail-info">
                        <span className="service-detail-price">{s.rate} ₽</span>
                        <span className="service-detail-unit">/ 1000 шт</span>
            ...
            {step === 'order' && selectedService && (
              <>
                ...
                <div className="quantity-control">
                  ...
                  <input type="number" value={quantity} onChange={(e) => { let val = parseInt(e.target.value) || (selectedService.min || 10); if (val < (selectedService.min || 10)) val = selectedService.min || 10; if (val > (selectedService.max || 100000)) val = selectedService.max || 100000; setQuantity(val) }} min={selectedService.min || 10} max={selectedService.max || 100000} />
                ...
                <div className="order-total">Итого к оплате: <span className="highlight">~{((selectedService.rate / 1000) * quantity).toFixed(2)} ₽</span></div>
        ...
        {activeTab === 'ref' && (
          <div className="profile">
            ...
            <div className="profile-balance" style={{ marginBottom: 12 }}>💰 Заработано: {refStats?.totalEarned || '0'} ₽</div>
            {refLink && (
              <>
